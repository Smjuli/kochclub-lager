...
app.get('/api/barcode/:code', (req, res) => {
  const a = db.data.artikel.find(x => x.barcode === req.params.code);
...
app.post('/api/artikel', (req, res) => {
  ...
  const neu = {
    ...
    name, barcode: barcode||'', kat: kat||'Sonstiges', ort: ort||'Regal A',
    menge: menge||0, einheit: einheit||'Stück', min_menge: min_menge||0, mhd,
...
app.put('/api/artikel/:id', (req, res) => {
  ...
  db.data.artikel[idx] = { ...db.data.artikel[idx], ...req.body, id: db.data.artikel[idx].id };
  db.save();
...
app.get('/api/buchungen', (req, res) => {
  ...
  const rows  = [...db.data.buchungen].reverse().slice(0, limit);
  res.json(rows);
});
...
// ─── EMAIL-WARNUNGEN (jeden Montag 08:00) ──────────────────────────
cron.schedule('0 8 * * 1', async () => {
  ...
  if (!smtpUser || !smtpPass || !mailTo) return; // nicht konfiguriert
  ...
  if (ablaufend.length) {
    text += 'BALD ABLAUFEND:\n';
    text += ablaufend.map(a => `  • ${a.name} – MHD: ${a.mhd}`).join('\n') + '\n\n';
  }
  if (knapp.length) {
    text += 'BESTAND KNAPP:\n';
    text += knapp.map(a => `  • ${a.name} – ${a.menge} ${a.einheit} (Mindest: ${a.min_menge})`).join('\n');
  }

  await transporter.sendMail({
    from: `"Kochclub Lager" <${smtpUser}>`,
    to: mailTo,
    subject: `[Kochclub] Lager-Warnung: ${ablaufend.length + knapp.length} Artikel beachten`,
    text
  });
  console.log('[Warnung] Mail gesendet:', new Date().toISOString());
});
